...
export function useAIAssistant() {
  return useMutation({
    mutationFn: ({ query, context }: { query: string; context: SimulationParameters }) =>
      api.askAIAssistant(query, context),
  })
}
...
export function useLatestRun() {
  return useQuery({
    ...
    refetchInterval: 5000, // Keep context fresh
  })
}

export function useGenerateAIReport() {
  return useMutation({
    mutationFn: (context: SimulationParameters) => api.generateAIReport(context),
  })
}
